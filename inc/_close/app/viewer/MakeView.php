...
class MakeView extends \Smarty {
	 ...
	 * @var array
	 */
	private static $smarty_dirs_not_providet = [
		'templates',
		'templates_c',
		'configs',
		'cache',
	...
	private function test_and_create_smarty_dirs() {
		...
		foreach(self::$smarty_dirs_not_providet as $dir) {
		...
		if($error === true) {
			...
			$errormsg = 'Please create the following directories in ' . $this->smarty_dir . ': <br>' . $errormsg
			            . '<br>' . 'Make sure that the directories are writable (0777)';
			die($errormsg);
		}
	}
